...
class QyWechat
{
     ...
     * @return bool
     */
    private function log($log, $p_type = 'INF')
    {
        if ((strtoupper($p_type) == 'ERR' || $this->debug) && function_exists($this->_logcallback)) {
            if (is_array($log)) $log = print_r($log, true);
            return call_user_func($this->_logcallback, $log, $p_type);
        }
    }
    ...
    public static function data_to_xml($data)
    {
        ...
        foreach ($data as $key => $val) {
            ...
            $xml .= "<$key>";
            $xml .= (is_array($val) || is_object($val)) ? self::data_to_xml($val) : self::xml_safe_str($val);
    ...
    private function http_get($url)
    {
        ...
        if (intval($aStatus["http_code"]) == 200) {
            ...
            $this->errCode = 100001;
            $this->errMsg = "发送GET请求时发生错误";
            $this->log($url . "|" . $this->errMsg, "ERR");
            return false;
        }
    }
    ...
    private function http_post($url, $param, $post_file = false)
    {
        ...
        if (intval($aStatus["http_code"]) == 200) {
            ...
            $this->errCode = 100002;
            $this->errMsg = "发送POST请求时发生错误,代码:" . $aStatus["http_code"];
            $this->log($url . "|" . $this->errMsg, "ERR");
            return false;
        }
    }
    ...
    public function valid($return = false)
    {
        ...
        if ($encryptStr) {
            $ret = $this->check_signature($encryptStr);
        }
        if (!isset($ret) || !$ret) {
            if (!$return) {
                die('no access');
     ...
     * @return type
     */
    private function generate_xml($encrypt, $signature, $timestamp, $nonce)
    {
    ...
    public function reply($msg = array(), $return = false)
    {
        ...
        $smsg = $this->generate_xml($encrypt, $signature, $timestamp, $nonce);
        $this->log($smsg);
        if ($return) {
            return $smsg;
        } elseif ($smsg) {
    ...
    {
        if ($this->_logcallback && function_exists($this->_logcallback)) {
            if (isset($p_value) && !empty($p_value)) {
                call_user_func($this->_cachecallback, $p_key, $p_value, $p_expires);
            } else {
                return call_user_func($this->_cachecallback, $p_key);
            }
        } else {
            return false;
        }
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '')
    {
        ...
        //获取当前缓存的access_token
        $t_cache = $this->cache_access_token($this->cacheKey);
        if ($t_cache) {
        ...
        if ($result) {
            ...
            //将access_token缓存
            $this->cache_access_token($this->cacheKey, $json['access_token'], $expire);
            return $this->access_token;
        }
        return false;
    }
    ...
    public function resetAuth($appid = '')
    {
        ...
        $this->access_token = '';
        //删除当前缓存的access_token
        $this->cache_access_token($this->cacheKey, null);
        return true;
    }
